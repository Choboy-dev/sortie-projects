...
const UNIFORMS = {
  // Dark tech fluid mesh: deep charcoal + brand terracotta (#964132) + warm highlights
  colors: [
    [0.039, 0.035, 0.031],
    [0.102, 0.059, 0.047],
    [0.588, 0.255, 0.196],
...
export function ShaderBackground({ className }: { className?: string }) {
  ...
  useEffect(() => {
    ...
    const compile = (type: number, src: string) => {
      const s = gpu.createShader(type)!
      gpu.shaderSource(s, src)
      gpu.compileShader(s)
      return s
    }
    const program = gpu.createProgram()!
    const vertexShader = compile(gpu.VERTEX_SHADER, VERT)
    const fragmentShader = compile(gpu.FRAGMENT_SHADER, FRAG)
    ...
    gpu.bindBuffer(gpu.ARRAY_BUFFER, buf)
    gpu.bufferData(
      gpu.ARRAY_BUFFER,
      new Float32Array([-1, -1, 3, -1, -1, 3]),
      gpu.STATIC_DRAW,
    )
    const loc = gpu.getAttribLocation(program, "a_position")
    gpu.enableVertexAttribArray(loc)
    gpu.vertexAttribPointer(loc, 2, gpu.FLOAT, false, 0, 0)

    const uni = {
      colors: gpu.getUniformLocation(program, "u_colors"),
      scene: gpu.getUniformLocation(program, "u_scene"),
      shape: gpu.getUniformLocation(program, "u_shape"),
      surface: gpu.getUniformLocation(program, "u_surface"),
      finish: gpu.getUniformLocation(program, "u_finish"),
      transform: gpu.getUniformLocation(program, "u_transform"),
      space: gpu.getUniformLocation(program, "u_space"),
      cursor: gpu.getUniformLocation(program, "u_cursor"),
    }
    gpu.uniform3fv(uni.colors, new Float32Array(UNIFORMS.colors.flat()))
    gpu.uniform4f(
      uni.shape,
      UNIFORMS.scale,
      UNIFORMS.intensity,
    ...
    return () => {
      ...
      gpu.deleteBuffer(buf)
      gpu.deleteProgram(program)
      const releaseTimer = window.setTimeout(() => {
        if (pendingContextReleases.get(surface) !== releaseTimer) return
        pendingContextReleases.delete(surface)
        gpu.getExtension("WEBGL_lose_context")?.loseContext()
        surface.width = 1
        surface.height = 1
      }, 0)
      pendingContextReleases.set(surface, releaseTimer)
    }
  }, [])
