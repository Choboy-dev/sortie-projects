...
const LogoImage = memo(function LogoImage({ logo }: { logo: Logo }) {
  return (
    // eslint-disable-next-line @next/next/no-img-element -- remote SVG wordmarks from 21st CDN
    <img
      alt={logo.alt}
      src={logo.src}
      width={logo.width}
      height={logo.height}
      loading="lazy"
      className="pointer-events-none h-7 w-auto max-w-[8.5rem] object-contain object-center select-none opacity-90 md:h-8"
...
export const LogoMarquee = memo(function LogoMarquee({
  ...
  return (
    <div
      className={cn(
        "mx-auto max-w-7xl overflow-hidden py-4 [mask-image:linear-gradient(to_right,transparent,black_25%,black_75%,transparent)]",
        className,
      )}
    >
